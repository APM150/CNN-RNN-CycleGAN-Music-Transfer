...
from models import Discriminator, RNNGenerator, LSTMGenerator,\
                    LSTMGeneratorV2, LSTMDiscriminator
...
class RNNTrainer:
    ...
    def init_model(self):
        ...
        if self.args.generator == "RNN":
            self.generatorAB = RNNGenerator(self.args).to(self.args.device)
            self.generatorBA = RNNGenerator(self.args).to(self.args.device)
        elif self.args.generator == "LSTMv1":
            self.generatorAB = LSTMGenerator(self.args).to(self.args.device)
            self.generatorBA = LSTMGenerator(self.args).to(self.args.device)
        elif self.args.generator == "LSTMv2":
            self.generatorAB = LSTMGeneratorV2(self.args).to(self.args.device)
            self.generatorBA = LSTMGeneratorV2(self.args).to(self.args.device)
            self.discriminatorA = LSTMDiscriminator(self.args).to(self.args.device)
            self.discriminatorB = LSTMDiscriminator(self.args).to(self.args.device)
        if self.args.model == "partial":
            ...
            if self.args.generator == "LSTMv2":
                self.discriminatorAM = LSTMDiscriminator(self.args).to(self.args.device)
                self.discriminatorBM = LSTMDiscriminator(self.args).to(self.args.device)
        # loss
